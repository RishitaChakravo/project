...
export default function DashBoard() {
  ...
  const renderContent = () => {
    switch (selected) {
      case "Create":
        return (
          <div className="bg-gray-800 p-6 rounded-xl shadow-lg w-full max-w-md mx-auto space-y-4 text-gray-200">
            ...
            <button className="w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 rounded-lg shadow transition"
      ...
      case "Display":
        return (
          <div className="grid gap-4">
            {tasks?.map((task) => (
              <div key={task._id} className="p-4 bg-gray-700 rounded-lg shadow-md hover:shadow-lg transition">
                ...
                <button className="mt-3 px-3 py-1 bg-red-600 hover:bg-red-700 text-white rounded-md transition"
                  onClick={async () => {
                    try {
        ...
        return (
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              {teams?.length ? (
                teams.map(team => (
                  <div key={team._id} className="p-4 bg-gray-700 rounded-lg shadow-md">
                    ...
                    <div className="space-y-2">
                      {team.members?.map(member => (
                        <div key={member._id} className="p-2 rounded bg-gray-800">
                          <div className="flex items-center justify-between">
                            <p className="font-medium">{member.name}</p>
                            {!member.tasks?.length && (
                              <button className="px-2 py-1 bg-green-600 text-white rounded hover:bg-green-700 text-sm"
                            ...
                            <div className="mt-2 space-y-1">
                              {member.tasks.map((t, idx) => (
                                <div key={t._id || idx} className="flex items-center justify-between text-sm">
                                  <p className={`${t.completed ? "line-through text-gray-400" : "text-gray-300"}`}>
                                    {t.description}
                                  </p>
                                  <button
  ...
  return (
    <div className="flex w-screen h-screen bg-gray-900 text-gray-200">
      {loading ? (
        ...
        <>
          <div className="w-1/4 p-6 flex flex-col bg-gray-800 shadow-xl border-r border-gray-700">
            <div className="mb-6">
            ...
            {["Create", "Display", "Team"].map(item => (
              <button key={item} className={`text-left px-4 py-2 mb-2 rounded-lg transition-colors ${selected === item ? "bg-blue-600 text-white font-semibold shadow-md" : "text-gray-300 hover:bg-gray-700 hover:text-white"}`}
                onClick={() => setSelected(item)}>{item}</button>
            ))}

            <button className="mt-auto px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg transition font-semibold"
              onClick={() => setLoggedIn(false)}>Logout</button>
          </div>

          <div className="flex-1 p-8 overflow-y-scroll">
            <h2 className="text-3xl font-extrabold mb-6 text-white border-b pb-2 border-gray-700">
              {selected}
            </h2>
